/**
 * /api/players-form.js
 * GET /api/players/:id/form — individual player form
 * GET /api/players/form — player form leaderboard
 */
...
export default async function handler(req, res) {
    ...
    try {
        if (id) {
            ...
            if (!history) {
                return res.status(200).json({
                    ...
                    team_name: player.teams ? player.teams.team_name : null,
                    raw_form: null,
                    decayed_form: null,
                    confidence: 'unranked',
                    trend: 'new',
            ...
            // GET /api/players/form (Leaderboard)
            const { data: players, error: playersErr } = await supabase
                .from('players')
                .select('id, professional_name, current_ign, teams(team_name)');
            ...
            // Map latest form entry for each player ID
            ...
            // Calculate live decay for all players
            const leaderboard = (players || []).map(player => {
                const history = latestHistoryMap[player.id];
                if (!history) {
                    return {
                        player_id: player.id,
                        professional_name: player.professional_name,
        ...
        console.error('[Players Form API] Error:', err);
